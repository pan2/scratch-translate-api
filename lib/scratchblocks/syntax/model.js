...
translate(lang, isShallow) {
    // --- DEBUG START ---
    console.log('[DEBUG] Block.translate called for ID:', this.info.id);
    if (typeof lang === 'object' && lang !== null && lang.commands) { // langがオブジェクトでcommandsを持つか確認
      console.log('[DEBUG] Block.translate: lang.name =', lang.name, '; lang.commands keys:', Object.keys(lang.commands).length);
    } else {
      console.error('[DEBUG] Block.translate: lang is problematic! Received:', lang, 'Type:', typeof lang); // 型情報も出力
    }
    // --- DEBUG END ---

    // langが文字列の場合、言語オブジェクトを取得する試み（ライブラリの標準動作ではないが、デバッグのため）
    if (typeof lang === 'string') {
      console.warn(`[DEBUG] Block.translate received lang as string: "${lang}". Attempting to get lang object.`);
      lang = allLanguages[lang]; // allLanguages を使ってオブジェクトを取得
      if (lang && lang.commands) {
        console.log('[DEBUG] Block.translate: Re-fetched lang.name =', lang.name, '; lang.commands keys:', Object.keys(lang.commands).length);
      } else {
        console.error('[DEBUG] Block.translate: Failed to re-fetch lang object or commands for string:', lang);
        return;
      }
    }

    if (!lang) {
      console.error("Block.translate received undefined/null lang object for block id:", this.info.id);
      return;
    }
    if (!lang.commands) {
        console.error("Block.translate: lang.commands is undefined for lang:", lang.name, "block id:", this.info.id);
        return;
    }
    ...
    // this.info.language はパース時の言語 (例: english)
    if (!this.info.language || !this.info.language.commands) {
        console.error("Block.translate: this.info.language or this.info.language.commands is undefined for block id:", this.info.id);
        return;
    }
